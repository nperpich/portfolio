...
// ---- Waypoints -------------------------------------------------------
// Starting points only — use "+ Add from view" in edit mode to build your
// real list, then copy the generated code block over this constant.
const DEFAULT_WAYPOINTS = [
  { pos: [2.4, 1.0, 2.4], look: [1.5, 0, 1.5] },
  { pos: [0, 1.4, 3.2], look: [1.5, 0, 1.5] },
  { pos: [-2.4, 0.8, 2.0], look: [1.5, 0, 1.5] },
  { pos: [-1.6, 0.4, -2.0], look: [1.5, 0.3, 1.5] },
  { pos: [1.2, 0.5, -2.4], look: [1.5, 0.3, 1.5] },
  { pos: [2.4, 1.0, 2.4], look: [1.5, 0, 1.5] },
];
...
export default function VenturePlugPage() {
  ...
  const orbitEnabled = !playing && (!editMode || selectedIndex === null);

  // Snap the live camera to the selected waypoint's exact pos/look.
  useEffect(() => {
  ...
  }, [editMode, selectedIndex, waypoints, playing]);

  const updateSelected = (field, axis, value) => {
    setWaypoints((wps) =>
      wps.map((w, i) => {
        if (i !== selectedIndex) return w;
        const next = { ...w, [field]: [...w[field]] };
  ...
  const nudge = (field, axis, dir) => {
    if (selectedIndex === null) return;
    const current = waypoints[selectedIndex][field][axis];
    updateSelected(field, axis, round(current + dir * nudgeStep));
  };
          ...
          {selectedIndex !== null && waypoints[selectedIndex] && (
            <div className="flex flex-col gap-3 border-t border-[#1c3a4a] pt-3">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1 text-xs text-[#9fc3d0]">
                  step
                  <select
                    value={nudgeStep}
                    onChange={(e) => setNudgeStep(parseFloat(e.target.value))}
                    className="bg-[#132735] border border-[#2a4a5c] rounded px-1"
                  >
              ...
              {['pos', 'look'].map((field) => (
                <div key={field} className="flex flex-col gap-1.5">
                  <span className="text-xs text-[#7fa6b3]">
                    {field === 'pos' ? 'Position' : 'Look at'}
                  </span>
                  {[0, 1, 2].map((axis) => (
                    <div key={axis} className="flex items-center gap-1.5">
                      ...
                      <button
                        onClick={() => nudge(field, axis, -1)}
                        className="w-6 h-6 rounded bg-[#132735] border border-[#2a4a5c] hover:border-[#4fd1e8]"
                      >
                        −
                      </button>
                      <input
                        type="number"
                        step={nudgeStep}
                        value={waypoints[selectedIndex][field][axis]}
                        onChange={(e) =>
                          updateSelected(
                            field,
                            axis,
                        ...
                        className="w-16 bg-[#132735] border border-[#2a4a5c] rounded px-1 py-0.5 font-mono text-xs text-center"
                      />
                      <button
                        onClick={() => nudge(field, axis, 1)}
                        className="w-6 h-6 rounded bg-[#132735] border border-[#2a4a5c] hover:border-[#4fd1e8]"
